//var dir = "http://www.yogic.net/ants/";
//dir = "file:///c:/personal/ants/";
var dir = window.foodir;
...
var isNewBrowser = ( isMinNS4 == 0 ) && ( isMinIE4 == 0 );
...
function endLoadBar()
{ // empty (can be changed) function called when finished
}
...
{
alert("move layer " + layer + " by " + dx + " " + dy + "   from " + layer.style.left + " " + layer.style.top);
   if (isMinNS4)
      layer.moveBy(dx, dy);
   if (isNewBrowser || isMinIE4)
   {
...
{
   var s, x, y;
//qqq   x = Math.floor(Math.random() * getWindowWidth());
   x = Math.floor(Math.random() * (getWindowWidth()-40));
//qqq   y = Math.floor(Math.random() * getWindowHeight());
   y = Math.floor(Math.random() * (getWindowHeight()-30));
   s = Math.floor(Math.random() * 4);
   if (s == 0)
      x = -getWidth(ants[n]);
   if (s == 1)
//qqq      x = getWindowWidth();
      x = getWindowWidth()-40;
   if (s == 2)
      y = -getHeight(ants[n]);
   if (s == 3)
//qqq      y = getWindowHeight();
      y = getWindowHeight()-30;
   x += getPageScrollX();
   y += getPageScrollY();
   moveLayerTo(ants[n], x, y);
}

function updateNextAnt()
{
   var i, dx, dy, theta, d;
   d = 3;
   i = (Math.random() * 7).toFixed(0);
//   for (i = 0; i < ants.length; i++)
   {
...
//qqq   setTimeout('updateAnts()', 50);
   setTimeout('updateNextAnt()', 10);
   return;
}
